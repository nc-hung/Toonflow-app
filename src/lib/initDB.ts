      ...
      initData: async (knex) => {
        await knex("o_agentDeploy").insert([
          ...
          },
          {
            ...
            name: "Agent Sản xuất: bản g Phân cảnh",
            desc: "Tạo bản g thông tin phân cảnh",
            temperature: 1,
            maxOutputTokens: 0,
            disabled: false,
          },
          {
            ...
            desc: "Tạo bản g dữ liệu phân cảnh",
            temperature: 1,
            maxOutputTokens: 0,
            disabled: false,
          },
      ...
      initData: async (knex) => {
        const list = [
          ...
          },
          {
            ...
            description: "Agent thực thi sản xuất: Trực tiếp tạo tài nguyên, bản g phân cảnh, prompt video.",
            embedding: "",
            type: "main",
            createTime: 1774447310118,
            updateTime: 1774447310118,
  ...
  for (const t of tables) {
    ...
    if (!tableExists || forceInit) {
      if (tableExists && forceInit) {
        ...
        console.log("[Khởi tạo CSDL] Đã xóa và tạo lại bản g:", t.name);
      } else {
        console.log("[Khởi tạo CSDL] Tạo bản g dữ liệu:", t.name);
      }
      await knex.schema.createTable(t.name, t.builder);
      if (t.initData) {
        await t.initData(knex);
        console.log("[Khởi tạo CSDL] Khởi tạo dữ liệu bản g:", t.name);
      }
    }
  }
};
